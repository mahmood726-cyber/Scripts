...
const TableDetector = {
    ...
    extractMediansFromLines(lines) {
        ...
        for (let i = 0; i < lines.length - 1; i++) {
            ...
            // Pattern: "Left ventricular ejection fraction" followed by "Median — % 32.5"
...
const CoreExtractor = {
    fields: {
        ...
        'baseline.sex.male_pct': {
            patterns: [/^male\s+sex/i, /^men\s*[-—]/i, /^male\b/i],
            valuePattern: /(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*\)/,
            captures: { 1: 'n', 2: 'pct' },
            validation: { pct: [0, 100] }
        },
    ...
    extract(rows, text) {
        ...
        rows.forEach(row => {
            ...
            if (/male\/female/i.test(row.label) && !results['baseline.sex.male_pct']) {
...
const OutcomesExtractor = {
    extract(text) {
        ...
        for (const primaryIdx of primaryLineIndices) {
            ...
            const searchEnd = Math.min(lines.length, primaryIdx + 8);  // Extended range for AFFIRM
            const contextLines = lines.slice(searchStart, searchEnd).join(' ');
            
            for (const pattern of hrPatterns.slice(0, 19)) {  // Include all HR patterns (v4.2: +2 for PARADIGM & MERIT-HF)
                pattern.lastIndex = 0;
                let match;
        ...
        if (!results.contrast) {
            for (const line of lines) {
                ...
                const isMainMortalityResult = lineLower.includes('mortality was lower') ||
                                              ...
                                              (lineLower.includes('deaths') && lineLower.includes('relative risk'));
                ...
                if (lineLower.includes('hazard ratio') || lineLower.includes('relative risk') || /\b(hr|rr)\s*[,=:]/i.test(line)) {
                    for (const pattern of hrPatterns.slice(0, 19)) {  // v4.2: Use all patterns including RR
                        pattern.lastIndex = 0;
                        ...
                        if (match) {
                            ...
                            if (effect >= 0.3 && effect <= 2.0 && ciLo < effect && effect < ciHi) {
                                results.contrast = {
                                    ...
                                    ciLo: ciLo,
                                    ciHi: ciHi,
                                    measureType: lineLower.includes('relative risk') ? 'RR' : 'HR',
                                    source: 'prose'
                                };
                                break;
                            }
...
const HFModule = {
    fields: {
        // Cardiac Function - EF
        'baseline.ef': {
            patterns: [/ejection\s+fraction/i, /^lvef\b/i, /^left\s+ventricular\s+ejection/i, /^lv\s+ejection/i],
    ...
    extract(rows, text) {
        ...
        // NYHA Class extraction from Roman numeral tables
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
            ...
            if (/new\s+york\s+heart\s+association/i.test(line) || /^nyha\s+(?:functional\s+)?class/i.test(line)) {
                ...
                for (let j = i + 1; j < Math.min(i + 10, lines.length); j++) {
                    ...
                    // Class I: "I 123 (5.2)" or "Class I 123 (5.2)" or "I 123 (5.2%)"
                    const class1Match = classLine.match(/^(?:Class\s+)?I\s+(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)/i);
                    if (class1Match && !results['baseline.nyha.class1_pct']) {
                        results['baseline.nyha.class1_pct'] = { n: parseInt(class1Match[1]), pct: parseFloat(class1Match[2]), source: 'table_nyha' };
                    }
                    
                    // Class II: "II 1600 (67.5)" or "II 811 (41%)"
                    const class2Match = classLine.match(/^(?:Class\s+)?II\s+(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)/i);
                    if (class2Match && !/III/.test(classLine) && !results['baseline.nyha.class2_pct']) {
                        results['baseline.nyha.class2_pct'] = { n: parseInt(class2Match[1]), pct: parseFloat(class2Match[2]), source: 'table_nyha' };
                    }
                    
                    // Class III: "III 750 (31.6)" or "III 1110 (56%)"
                    const class3Match = classLine.match(/^(?:Class\s+)?III\s+(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)/i);
                    if (class3Match && !results['baseline.nyha.class3_pct']) {
                        results['baseline.nyha.class3_pct'] = { n: parseInt(class3Match[1]), pct: parseFloat(class3Match[2]), source: 'table_nyha' };
                    }
                    
                    // Class IV: "IV 15 (0.6)" or "IV 69 (3.4%)"
                    const class4Match = classLine.match(/^(?:Class\s+)?IV\s+(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)/i);
                    if (class4Match && !results['baseline.nyha.class4_pct']) {
                        results['baseline.nyha.class4_pct'] = { n: parseInt(class4Match[1]), pct: parseFloat(class4Match[2]), source: 'table_nyha' };
                    }
                }
            ...
            for (let i = 0; i < ntprobnpPatterns.length - 1; i++) {
                ...
                if (match) {
                    results['baseline.ntprobnp'] = {
                        median: parseInt(match[1]),
                        iqrLo: parseInt(match[2]),
                        iqrHi: parseInt(match[3]),
                        source: 'prose_hf'
                    };
                    break;
                }

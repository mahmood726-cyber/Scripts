/**
 * RCT EXTRACTION ENGINE v4.6
 * ==========================
 * 
 * v4.6 Updates (AFFIRM-AHF + SOLVD-Prevention + ATLAS validation):
 * - AFFIRM-AHF: Added "in the X group and Y in the placebo group" arm N pattern
 * - AFFIRM-AHF: Added "Age, years: 71.2 (10.8)" comma format
 * - AFFIRM-AHF: Added "LVEF, %: 32.6% (9.6)" comma format
 * - AFFIRM-AHF: Added "Sex Male: 314 (56%)" pattern
 * - AFFIRM-AHF: Added "Class II: 255 (46%)" NYHA colon format
 * - AFFIRM-AHF: Added "rate ratio [RR] 0.79" pattern
 * - AFFIRM-AHF: Added "52 weeks" follow-up pattern (converted to months)
 * - SOLVD-Prevention: Added "Age: 59.1 years" prose pattern
 * - SOLVD-Prevention: Added plain EF decimal "0.28" without SD pattern
 * - ATLAS: Added "n=1596" and "n=1568" arm N pattern
 * - ATLAS: Added "0.23±0.06" EF decimal ± format
 * - Added ferric carboxymaltose and lisinopril to DRUG_NAMES
 * - Added AFFIRM-AHF, SOLVD-Prevention trial detection patterns
 * 
 * v4.5 Updates (SOLVD-Treatment + CONSENSUS validation - ACE-I trials 1987-1991):
 * - SOLVD-Treatment: Added "follow-up averaged X months" pattern
 * - SOLVD-Treatment: Added "Age (yr)" plain value pattern (no SD)
 * - SOLVD-Treatment: Added "Ejection fraction (%)" plain value pattern
 * - SOLVD-Treatment: Added "reduction in risk, X percent; 95 percent CI" pattern for RR
 * - SOLVD-Treatment: Reordered detectRows patterns to prioritize plain decimal numbers
 * - SOLVD-Treatment: Added plain percentage value extraction for Digitalis
 * - CONSENSUS: Added "X (Y mg)" dose format to detectRows patterns
 * - CONSENSUS: Added MRA pattern for "Spironolactone (mean dose)" format
 * - CONSENSUS: Added "reduction of X percent (P=Y)" pattern for RR (no CI)
 * 
 * v4.4 Updates (DIG + Val-HeFT + SOLOIST-WHF + CIBIS-II + SHIFT + CHARM-Added validation):
 ...
 * - CHARM-Added: Added Candesartan to drug list
 * - CHARM-Added: Added "Mean (SD) age" pattern for age extraction
 * - CHARM-Added: Added "Men/" pattern for male percentage
 * - CHARM-Added: Added "Mean (SD) LVEF" pattern for EF
 * - CHARM-Added: Added standalone spironolactone/eplerenone to MRA patterns
 * 
...
const TableDetector = {
    DRUG_NAMES: [
        ...
        'Sacubitril', 'Valsartan', 'Candesartan', 'LCZ696', 'Entresto',  // v4.4: Added Candesartan for CHARM-Added
        'Omecamtiv', 'Vericiguat', 'Finerenone', 'Eplerenone', 'Spironolactone',
        'Enalapril', 'Lisinopril', 'Ivabradine', 'Digoxin',  // v4.6: Added Lisinopril for ATLAS
        // v4.6: Iron therapy for AFFIRM-AHF
    ...
    detectRows(lines) {
        ...
        const valuePatterns = [
            ...
            // v4.4: SOLVD format - plain decimal numbers at end: "Age (yr)  61.0  60.7"
            // Must come BEFORE the yr|%|kg pattern to get correct label
            /\s+\d+\.\d+\s+\d+\.\d+$/,
            /\s\d{1,3}\s+\d{1,3}\s*$/,
            ...
            /\d{1,2}\.\d+\s*\(\s*\d+\.\d+\s*\)/,
            // v4.4: CONSENSUS format: "55 (80 mg)" - dose with mg unit
            /\d{1,3}\s*\(\s*\d+\s*mg\s*\)/,
    ...
    extractArmNsFromProse(text) {
        ...
        // v4.6: AFFIRM-AHF format: "558 in the carboxymaltose group and 550 in the placebo group"
        const inGroupMatch = text.match(/(\d+)\s+in\s+the\s+(\w+(?:\s+\w+)?)\s+group\s+and\s+(\d+)\s+in\s+the\s+placebo\s+group/i);
        if (inGroupMatch) {
            if (!results['arm.treatment.n']) {
                results['arm.treatment.n'] = { n: parseInt(inGroupMatch[1]), drug: inGroupMatch[2], source: 'prose_in_group' };
            }
            if (!results['arm.control.n']) {
                results['arm.control.n'] = { n: parseInt(inGroupMatch[3]), source: 'prose_in_group' };
            }
        }
        
        // v4.6: ATLAS format: "n=1596" and "n=1568" or "(n=X)"
        const nEqualsPatterns = text.matchAll(/(?:low[- ]?dose|high[- ]?dose|treatment|active)(?:[^,]*?),?\s*n\s*=\s*(\d+)/gi);
        const nEqualsMatches = [...nEqualsPatterns];
        if (nEqualsMatches.length >= 2) {
            if (!results['arm.treatment.n']) {
                results['arm.treatment.n'] = { n: parseInt(nEqualsMatches[1][1]), source: 'prose_n_equals' };
            }
            if (!results['arm.control.n']) {
                results['arm.control.n'] = { n: parseInt(nEqualsMatches[0][1]), source: 'prose_n_equals' };
            }
        }
...
const CoreExtractor = {
    fields: {
        // Demographics
        'baseline.age': {
            patterns: [/^age\s*[-—]/i, /^age\b/i, /^median\s+age/i, /^mean\s*\(range\)\s*age/i, /^age\s*\(years?\)/i, /^mean\s*\(SD\)\s*age/i],
            valuePattern: /(\d+(?:\.\d+)?)\s*[±+]\s*(\d+(?:\.\d+)?)/,
            captures: { 1: 'mean', 2: 'sd' },
            validation: { mean: [18, 100], sd: [1, 30] }
        },
        // v4.4: SHIFT/CHARM-Added format: "Age (years) 60.7 (11.2)" or "Mean (SD) age (years) 64.0 (10.7)" - SD in parentheses
        'baseline.age.paren_sd': {
            patterns: [/^age\s*\(years?\)/i, /^mean\s*\(SD\)\s*age/i],
        ...
        // v4.4: SOLVD format: "Age (yr) 61.0" - plain value without SD
        'baseline.age.plain': {
            patterns: [/^age\s*\(yr\)/i],
            valuePattern: /(\d+(?:\.\d+)?)\s+\d/,  // match first number followed by space and another number
            captures: { 1: 'mean' },
            mapTo: 'baseline.age',
            validation: { mean: [18, 100] }
        },
        // v4.6: AFFIRM-AHF format: "Age, years 71.2 (10.8)" or "Age, years: 71.2 (10.8)"
        'baseline.age.comma_years': {
            patterns: [/^age,\s*years?$/i, /^age,\s*years?:/i],
            valuePattern: /(\d+(?:\.\d+)?)\s*\(\s*(\d+(?:\.\d+)?)\s*\)/,
            captures: { 1: 'mean', 2: 'sd' },
            mapTo: 'baseline.age',
            validation: { mean: [18, 100], sd: [1, 30] }
        },
        // v4.6: SOLVD-Prevention format: "59.1 years" plain value
        'baseline.age.plain_years': {
            patterns: [/^age:/i],
            valuePattern: /(\d+(?:\.\d+)?)\s*years?/,
            captures: { 1: 'mean' },
            mapTo: 'baseline.age',
            validation: { mean: [18, 100] }
        },
        ...
        'baseline.sex.male_pct': {
            patterns: [/^male\s+sex/i, /^men\s*[-—]/i, /^male\b/i, /^sex\s*\(male\)/i, /^men\s*\//i, /^sex\s+male:/i],  // v4.6: Added "Sex Male:"
        ...
        'baseline.meds.bb_pct': {
            ...
            valuePattern: /(?:(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)|(\d+(?:\.\d+)?)\s*%)/,  // v4.6: Added plain pct with % for SOLVD-Prevention
            captures: { 1: 'n', 2: 'pct', 3: 'pct' }
        },
...
const OutcomesExtractor = {
    extract(text) {
        ...
        const hrPatterns = [
            ...
            // v4.6: AFFIRM-AHF format: "rate ratio [RR] 0.79, 95% CI 0.62–1.01" or "(RR 0.79, 95% CI 0.62–1.01)"
            /rate\s+ratio\s*\[?RR\]?\s*(\d+[·.]\d+)[,;]?\s*(?:97\.5%?\s*|95%?\s*)?CI[,:\s]*(\d+[·.]\d+)\s*[-–]\s*(\d+[·.]\d+)/gi,
            /\(RR\s+(\d+[·.]\d+)[,;]?\s*(?:97\.5%?\s*|95%?\s*)?CI[,:\s]*(\d+[·.]\d+)\s*[-–]\s*(\d+[·.]\d+)/gi,
            // v4.6: HR X.XX format with parentheses: "(HR 0.80, 95% CI 0.66–0.98)"
            /\(HR\s+(\d+[·.]\d+)[,;]?\s*(?:97\.5%?\s*|95%?\s*)?CI[,:\s]*(\d+[·.]\d+)\s*[-–]\s*(\d+[·.]\d+)/gi,
        ...
        const primaryKeywords = [
            ...
            'primary efficacy', 'for the primary', 'primary events'  // v4.6: AFFIRM-AHF format
        ];
        ...
        // v4.4: SOLVD format: "reduction in risk, 16 percent; 95 percent confidence interval, 5 to 26 percent"
        // v4.6: Also handle "risk reduction, 29 percent" format for SOLVD-Prevention
        // v4.6: Prioritize HF-related outcomes when multiple matches exist
        if (!results.contrast) {
            const solvdPatterns = [
                /reduction\s+in\s+risk,?\s*(\d+)\s*percent;\s*(?:97\.5|95)\s*percent\s+confidence\s+interval,?\s*(\d+)\s*to\s*(\d+)\s*percent/gi,
                /risk\s+reduction,?\s*(\d+)\s*percent;\s*(?:97\.5|95)\s*percent\s+confidence\s+interval,?\s*(\d+)\s*to\s*(\d+)\s*percent/gi  // v4.6: SOLVD-Prevention
            ];
            
            let allMatches = [];
            for (const solvdPattern of solvdPatterns) {
                let match;
                while ((match = solvdPattern.exec(text)) !== null) {
                    const reductionPct = parseInt(match[1]);
                    const ciLoPct = parseInt(match[2]);
                    const ciHiPct = parseInt(match[3]);
                    
                    // Get context around match to check for HF keywords
                    const contextStart = Math.max(0, match.index - 100);
                    const contextEnd = Math.min(text.length, match.index + 100);
                    const context = text.substring(contextStart, contextEnd).toLowerCase();
                    
                    const isHFRelated = context.includes('heart failure') || 
                                        context.includes('chf') || 
                                        context.includes('hospitalization');
                    
                    const effect = 1 - (reductionPct / 100);
                    const ciLo = 1 - (ciHiPct / 100);
                    const ciHi = 1 - (ciLoPct / 100);
            ...
            // Prioritize HF-related outcomes, then largest reduction
            if (allMatches.length > 0) {
                allMatches.sort((a, b) => {
                    if (a.isHFRelated && !b.isHFRelated) return -1;
                    if (!a.isHFRelated && b.isHFRelated) return 1;
                    return b.reduction - a.reduction;  // Larger reduction = lower RR = better
                });
        ...
        // v4.4: CONSENSUS format: "a reduction of 40 percent (P=0.002)" - no CI, just P value
        // Look for 6-month mortality context first
        if (!results.contrast) {
            const consensusPattern = /(?:six[- ]?month|6[- ]?month).*?reduction\s+of\s+(\d+)\s*percent\s*\(\s*P\s*[=<]\s*([\d.]+)\s*\)/i;
            const match = text.match(consensusPattern);
            if (match) {
                const reductionPct = parseInt(match[1]);
                const pValue = parseFloat(match[2]);
                
                // Convert: 40% reduction = RR of 0.60
                const effect = 1 - (reductionPct / 100);
                
                if (effect >= 0.3 && effect <= 1.0 && pValue < 0.05) {
                    results.contrast = {
                        effect: Math.round(effect * 100) / 100,
                        pValue: pValue,
                        measureType: 'RR',
                        source: 'prose_reduction_consensus'
                    };
                }
            }
        }
...
const ProseExtractor = {
    extract(text) {
        ...
        const hfTrialPatterns = [
            ...
            // v4.6: AFFIRM-AHF must come before SOLVD to avoid SOLVD-Prevention confusion
            { pattern: /\bAFFIRM[- ]?AHF\b/i, name: 'AFFIRM-AHF' },
            { pattern: /ferric\s+carboxymaltose.*acute\s+heart\s+failure/i, name: 'AFFIRM-AHF' },
            // v4.6: Distinguish SOLVD-Prevention from SOLVD-Treatment
            { pattern: /\bSOLVD\b.*prevention/i, name: 'SOLVD-Prevention' },
            { pattern: /\bSOLVD\b.*treatment/i, name: 'SOLVD-Treatment' },
        ...
        const followUpPatterns = [
            ...
            // v4.4: SOLVD format: "The follow-up averaged 41.4 months"
            { pattern: /follow[- ]?up\s+averaged\s+(\d+(?:\.\d+)?)\s*(years?|months?)/i, priority: 1 },
            ...
            // v4.6: AFFIRM-AHF format: "up to 52 weeks" or "52 weeks after randomisation"
            { pattern: /up\s+to\s+(\d+)\s*(weeks?)/i, priority: 2 },
            { pattern: /(\d+)\s*(weeks?)\s+(?:of\s+)?follow[- ]?up/i, priority: 2 },
            { pattern: /(\d+)\s*(weeks?)\s+after\s+(?:randomisation|randomization)/i, priority: 2 }
        ];
        ...
        for (const { pattern, priority } of followUpPatterns) {
            ...
            if (match && priority < bestPriority) {
                ...
                if (unit.startsWith('year') && duration < 10) {
                    ...
                    // v4.6: Convert weeks to months (52 weeks = 12 months)
                    bestMatch = { duration: Math.round(duration / 4.33), unit: 'months' };
                } else {
...
const HFModule = {
    fields: {
        // Cardiac Function - EF
        'baseline.ef': {
            patterns: [/ejection\s+fraction/i, /^lvef\b/i, /^left\s+ventricular\s+ejection/i, /^lv\s+ejection/i, /left-ventricular\s+ejection/i, /^mean\s*\(SD\)\s*lvef/i],
            valuePattern: /(\d{1,2}(?:\.\d+)?)\s*[±+]\s*(\d+(?:\.\d+)?)/,
            captures: { 1: 'mean', 2: 'sd' },
            validation: { mean: [5, 75], sd: [1, 20] },
            critical: true
        },
        // v4.4: SHIFT/CHARM-Added format: "LVEF (%) 29.0% (5.1)" or "Mean (SD) LVEF (%) 28.0 (7.5)" - SD in parentheses
        'baseline.ef_pct_paren': {
            patterns: [/^lvef\s*\(%\)/i, /ejection\s+fraction\s*\(%\)/i, /^mean\s*\(SD\)\s*lvef/i],
        ...
        // v4.4: SOLVD format: "Ejection fraction (%) 24.9" - plain value without SD
        'baseline.ef_plain': {
            patterns: [/^ejection\s+fraction\s*\(%\)/i],
            valuePattern: /(\d{1,2}(?:\.\d+)?)\s+\d/,  // match first number followed by another
            captures: { 1: 'mean' },
            validation: { mean: [5, 75] },
            mapTo: 'baseline.ef'
        },
        // v4.6: AFFIRM-AHF format: "LVEF, % 32.6% (9.6)" or "LVEF, %: 32.6% (9.6)"
        'baseline.ef_comma_pct': {
            patterns: [/^lvef,\s*%$/i, /^lvef,\s*%:/i],
            valuePattern: /(\d{1,2}(?:\.\d+)?)\s*%?\s*\(\s*(\d+(?:\.\d+)?)\s*\)/,
            captures: { 1: 'mean', 2: 'sd' },
            validation: { mean: [5, 75], sd: [1, 20] },
            mapTo: 'baseline.ef'
        ...
        'baseline.nyha.class1_pct': {
            patterns: [/nyha.*class\s*i(?:\s|:|$)/i, /^class\s*i(?:\s|:|$)/i, /^i\s+\d+\s*\(/, /^i$/i],  // v4.6: Added standalone "I" for SOLVD-Prevention
            valuePattern: /(?:(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)|(\d+(?:\.\d+)?)\s*%)/,  // v4.6: Added plain pct "66.7%" format
            captures: { 1: 'n', 2: 'pct', 3: 'pct' }
        },
        'baseline.nyha.class2_pct': {
            patterns: [/nyha.*class\s*ii(?:\s|:|$)/i, /^class\s*ii(?:\s|:|$)/i, /^ii\s+\d+\s*\(/, /^ii$/i],  // v4.6: Added standalone "II"
            valuePattern: /(?:(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)|(\d+(?:\.\d+)?)\s*%)/,  // v4.6: Added plain pct
            captures: { 1: 'n', 2: 'pct', 3: 'pct' },
            critical: true
        },
        'baseline.nyha.class3_pct': {
            patterns: [/nyha.*class\s*iii(?:\s|:|$)/i, /^class\s*iii(?:\s|:|$)/i, /^iii\s+\d+\s*\(/],  // v4.6: Added colon
            valuePattern: /(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)/,  // v4.6: Added %?
            captures: { 1: 'n', 2: 'pct' },
            critical: true
        },
        'baseline.nyha.class4_pct': {
            patterns: [/nyha.*class\s*iv(?:\s|:|$)/i, /^class\s*iv(?:\s|:|$)/i, /^iv\s+\d+\s*\(/],  // v4.6: Added colon
            valuePattern: /(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)/,  // v4.6: Added %?
            captures: { 1: 'n', 2: 'pct' }
        },
        ...
        'baseline.meds.arni_pct': {
            ...
            // v4.6: Added long form "Angiotensin receptor neprilysin inhibitor" (AFFIRM-AHF format)
            patterns: [/sacubitril/i, /arni?\b/i, /^with\s+neprilysin/i, /lcz696/i, /entresto/i, /arn\s+inhibitor/i, /angiotensin\s+receptor\s+neprilysin/i],
            valuePattern: /(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)/,  // v4.6: Added %? for AFFIRM-AHF
            captures: { 1: 'n', 2: 'pct' }
        },
        'baseline.meds.mra_pct': {
            patterns: [
                ...
                /^mra\b/i, 
                /^spironolactone\b/i,  // v4.4: Standalone spironolactone for CHARM-Added
                /^eplerenone\b/i,      // v4.4: Standalone eplerenone
                /spironolactone.*eplerenone/i,
                /eplerenone.*spironolactone/i,
                /antialdosterone/i  // v4.4: SHIFT format
            ],
            valuePattern: /(?:(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)|(\d+)\s*\(\s*\d+\s*mg\s*\)|(\d+)\s+\d)/,  // v4.4: CONSENSUS "55 (80 mg)" and plain formats
            captures: { 1: 'n', 2: 'pct', 3: 'pct', 4: 'pct' },
            critical: true
        },
        ...
        'baseline.meds.digoxin_pct': {
            ...
            valuePattern: /(?:(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)|(\d+(?:\.\d+)?)\s*%|(\d+(?:\.\d+)?)\s+\d)/,  // v4.6: Added plain pct with % for SOLVD-Prevention
            captures: { 1: 'n', 2: 'pct', 3: 'pct', 4: 'pct' }
        },
        ...
        'baseline.device.crt_pct': {
            ...
            // v4.6: Handle British spelling "resynchronisation" (AFFIRM-AHF format)
            patterns: [/cardiac[\s-]+resynchroni[sz]ation/i, /^crt\b/i, /biventricular/i, /crt-?d/i, /crt-?p/i],
            valuePattern: /(\d+)\s*\(\s*(\d+(?:\.\d+)?)\s*%?\s*\)/,  // v4.6: Added %? for AFFIRM-AHF format
            captures: { 1: 'n', 2: 'pct' }
        },
    ...
    extract(rows, text) {
        ...
        if (!results['baseline.age']) {
            const ageProsePatterns = [
                ...
                // v4.6: SOLVD-Prevention format: "Age: 59.1 years"
                /^age:\s*(\d+(?:\.\d+)?)\s*years/im
            ];
        ...
        if (!results['baseline.ef']) {
            for (let i = 0; i < lines.length; i++) {
                ...
                // v4.6: ATLAS format: "Left ventricular ejection fraction: 0.23±0.06"
                const decimalPlusMinusMatch = line.match(/ejection\s+fraction[:\s]+(0[·.]\d{2})\s*[±+]\s*(0[·.]\d{2})/i);
                if (decimalPlusMinusMatch) {
                    const meanDecimal = parseFloat(decimalPlusMinusMatch[1].replace('·', '.'));
                    const sdDecimal = parseFloat(decimalPlusMinusMatch[2].replace('·', '.'));
                    const mean = Math.round(meanDecimal * 100);  // 0.23 -> 23
                    const sd = Math.round(sdDecimal * 100);      // 0.06 -> 6
                    if (mean >= 5 && mean <= 75 && sd >= 1 && sd <= 20) {
                        results['baseline.ef'] = { mean, sd, source: 'decimal_plusminus_hf' };
                        break;
                    }
                }
                // v4.6: SOLVD-Prevention format: "mean left ventricular ejection fraction was 0.28" (plain decimal without SD)
                const plainDecimalMatch = line.match(/ejection\s+fraction[:\s]+(?:was\s+)?(0[·.]\d{2})(?:\s|$|;)/i);
                if (plainDecimalMatch) {
                    const meanDecimal = parseFloat(plainDecimalMatch[1].replace('·', '.'));
                    const mean = Math.round(meanDecimal * 100);  // 0.28 -> 28
                    if (mean >= 5 && mean <= 75) {
                        results['baseline.ef'] = { mean, source: 'plain_decimal_hf' };
                        break;
                    }
                }
            }
